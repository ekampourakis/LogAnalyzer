...
Public Class Main
    ...
    Public LogFile As New Log()

    Private Sub Main_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Log.Write("Starting")
        OpenLogFile("D:\Cloud\OneDrive - ceid.upatras.gr\Formula\log - Copy_Run1.txt")
    End Sub
...
#Region "Log IO"
    ...
    Private Sub OpenLogFile(ByVal Path As String)
        ClearCharts()
        Log.Write("Loading log file: " & Path)
        CheckedListBox.Items.Clear()
        If LogFile.Open(Path) Then
            ToolStripStatusLabel_LogFile.Text = Path
            CheckedListBox.Items.AddRange(LogFile.Headers)
            ...
            End If
            MaxScaleSize = LogFile.Length
            ScaleSize = LogFile.Length
        Else
    ...
    Private Sub SaveFileDialog_FileOk(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles SaveFileDialog.FileOk
        ...
        'Writer.Close()
        'Writer.Dispose()
        LogFile.Save(SaveFileDialog.FileName)
    End Sub
...
#Region "Chart Loading"
    ...
    Private Sub SetChartArea(ByRef Area As ChartArea)
        ...
        Area.AxisX.ScaleView.Size = LogFile.Length 'LogFileLength
        ...
        Try
            ...
            For Index As Integer = 1 To CheckedListBox.Items.Count
                ...
                If CheckedListBox.GetItemChecked(Index - 1) Then
                    ...
                    Dim SeriesName As String = CheckedListBox.Items(Index - 1)

                    ' Create the new chart area
                    Dim Area As New ChartArea
                    Area.Name = "ChartArea_" & SeriesName
                    ...
                    Legend.Name = "Legend_" & SeriesName
                    ...
                    Series.Name = "Series_" & SeriesName
                    ...
                    Series.LegendText = SeriesName & " Min: #MIN{D0}, Max: #MAX{D0}"
                    ...
                    Dim Millis As Integer = LogFile("Time").Item(0) 'Line(0)
                    Dim LastValue As Double = LogFile(SeriesName).Item(0) 'CDbl(Line(Index))
                    ...
                    For Index2 As Integer = 2 To LogFile.Count() - 1
                        Millis = LogFile("Time").Item(Index2)
                        If Millis < LastMillis Then
                            ...
                            End If
                        End If
                        If LogFile(SeriesName).Item(Index2) <> LastValue Or Index2 = LogFile.Count - 1 Then
                            'LastValue = CDbl(Line(Index))
                            LastValue = LogFile(SeriesName).Item(Index2)
                            'Series.Points.AddXY(ChartTimeOffset.AddMilliseconds(Millis), LastValue)
                            Series.Points.AddXY(Millis, LastValue)
                        Else
                            Avoided += 1
...
#Region "Chart Menu"
    ...
    Private Sub ResetScaleToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ResetScaleToolStripMenuItem.Click
        ZoomChart(LogFile.Length, LogFile.MinTime) 'LogFile(1).Split(LogDelimiter)(0))
    End Sub
    ...
    Private Sub DataEditorToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DataEditorToolStripMenuItem.Click
        If LogFile IsNot Nothing And LogFile.Headers IsNot Nothing Then DataEditor.Show()
    End Sub

    Private Sub ExportToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExportToolStripMenuItem.Click
        SaveFileDialog.FileName = "Export" 'LogPath.Substring(LogPath.LastIndexOf("\") + 1)
...
#Region "Data Editing"
    ...
    Private Sub TrimLog()
        ...
        '    End If
        'End If
        If LogFile.Trim(SelectionStart, SelectionEnd) Then
            DataUpdated()
        End If
    End Sub
